...
<div class="card">
  <div class="card-header">
    <span class="card-title">👥 Band Members</span>
    <button class="btn btn-primary btn-sm" data-modal="modalAdd">+ Add Member</button>
  </div>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="text-sm text-muted"><?= h($m['email']) ?></td>
          <td><?= roleBadge($m['role']) ?></td>
          <td><?= h($m['instrument'] ?: '—') ?></td>
          <td><?= h($m['year_level'] ?: '—') ?></td>
          <td class="text-sm"><?= h($m['student_id'] ?: '—') ?></td>
          <td><?= statusBadge($m['status']) ?></td>
          <td class="<?= penaltyColor((float)$m['penalty_points']) ?> text-bold"><?= $m['penalty_points'] ?></td>
          <td>
            <button class="btn btn-xs btn-outline"
              data-modal="modalEdit"
              onclick="fillEdit(<?= htmlspecialchars(json_encode($m), ENT_QUOTES) ?>)">Edit</button>
            <?php if ($m['id'] !== $user['id']): ?>
            <form method="POST" style="display:inline">
              <input type="hidden" name="action" value="delete">
              <input type="hidden" name="id" value="<?= $m['id'] ?>">
              <button type="submit" class="btn btn-xs btn-danger" data-confirm="Delete <?= h($m['name']) ?>?">Del</button>
            </form>
            <?php endif; ?>
          </td>
        </tr>
        <?php endforeach; ?>
        <?php if (!$members): ?>
        <tr><td colspan="9"><div class="empty-state"><div class="empty-icon">👥</div><p>No members yet.</p></div></td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
  ...
  <div class="modal">
    ...
    <form method="POST">
      ...
      <div class="modal-body">
        ...
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Password</label><input name="password" class="form-control" placeholder="Default: password"></div>
        ...
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Instrument</label><input name="instrument" class="form-control"></div>
          <div class="form-group"><label class="form-label">Year Level</label><input name="year_level" class="form-control" placeholder="e.g. 2nd Year"></div>
        </div>
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Student ID</label><input name="student_id" class="form-control"></div>
          <div class="form-group"><label class="form-label">Contact</label><input name="contact_number" class="form-control"></div>
        </div>
      </div>
...
<div class="modal-overlay" id="modalEdit">
  <div class="modal">
    ...
    <form method="POST">
      ...
      <div class="modal-body">
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Full Name *</label><input id="edit_name" name="name" class="form-control" required></div>
          <div class="form-group"><label class="form-label">Email *</label><input id="edit_email" name="email" type="email" class="form-control" required></div>
        </div>
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Role</label>
        ...
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Instrument</label><input id="edit_instrument" name="instrument" class="form-control"></div>
          <div class="form-group"><label class="form-label">Year Level</label><input id="edit_year_level" name="year_level" class="form-control"></div>
        </div>
        <div class="form-row form-row-2">
          <div class="form-group"><label class="form-label">Student ID</label><input id="edit_student_id" name="student_id" class="form-control"></div>
          <div class="form-group"><label class="form-label">Contact</label><input id="edit_contact_number" name="contact_number" class="form-control"></div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-outline" data-modal-close>Cancel</button>
